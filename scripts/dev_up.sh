...
derive_live_submit_plan() {
  ...
  base_plan="$(first_existing_plan \
    plans/dev.tqkq_live_submit.json \
    plans/dev.tqkq_live_dryrun_min_loop.json \
    plans/dev.tqkq_live_dryrun.json)" || {
      echo "missing base tqkq live plan" >&2
      return 1
    }
  ...
  TMP_PLAN="$(mktemp "${TMPDIR:-/tmp}/futures_v2_tqkq_live_submit.${runtime_id}.XXXXXX")" || {
...
plan["adapters"]["broker"]["mode"] = "tqkq_live"
...
case "$MODE" in
  live_file)
    PLAN="$(first_existing_plan plans/dev.live_file_bars.json plans/dev.live_file.json)" || {
      echo "missing live_file plan" >&2
      exit 1
    }
    WRITER_ENABLED=1
    ;;
  tqkq_dryrun)
    PLAN="$(first_existing_plan plans/dev.tqkq_live_dryrun_min_loop.json plans/dev.tqkq_live_dryrun.json)" || {
      echo "missing tqkq dry-run plan" >&2
      exit 1
    }
    WRITER_ENABLED=0
    ;;
