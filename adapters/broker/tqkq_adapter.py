...
class TqKqBrokerAdapter(BrokerAdapter):
    """
    TqKq live broker.
    ...
    object. Tests inject an API double; no contract depends on network access.
    """
    ...
    def __init__(
        ...
        *,
        ...
        order_id_prefix: str = "TQKQ-ORDER",
    ...
    def _api_instance(self) -> Any:
        ...
        if self._api_factory is None:
            raise ValueError("TQKQ broker requires api_factory when dry_run=false")
    ...
    def _submit_native_order(self, order: ExecutionOrder) -> Any:
        ...
        if not callable(insert_order):
            raise ValueError("TQKQ api missing insert_order")
        limit_price = self._limit_price(order)
        return insert_order(
    ...
    def _limit_price(self, order: ExecutionOrder) -> float:
        price = order.price
        if price is None:
            price = self.market_data.get_last_quote(order.instrument_id).price
        ...
        if order.side == Side.BUY:
            return ceil(units) * tick
        if order.side == Side.SELL:
            return floor(units) * tick
...
def _direction(side: Side) -> str:
    ...
    if side == Side.SELL:
        return "SELL"
    raise ValueError(f"unsupported side for TQKQ: {side}")
