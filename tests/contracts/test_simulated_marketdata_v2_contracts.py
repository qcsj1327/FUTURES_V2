...
def test_simulated_v2_advances_and_is_deterministic() -> None:
    ...
    p1a = md1.get_last_prices(["au", "ag"])
    p2a = md2.get_last_prices(["au", "ag"])
    assert p1a == p2a

    md1.advance()
    md2.advance()

    p1b = md1.get_last_prices(["au", "ag"])
    p2b = md2.get_last_prices(["au", "ag"])
    assert p1b == p2b
    assert p1b != p1a
