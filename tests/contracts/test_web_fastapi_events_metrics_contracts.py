...
def test_fastapi_events_and_metrics_routes(
    tmp_path: Path,
    ...
    r = client.get(f"/runs/{rid}/events?env=live&tail=10")
    assert r.status_code == 200
    payload = r.json()
    assert payload["runtime_id"] == rid
    assert payload["env"] == "live"
    assert isinstance(payload["fill_events"], list)
    if payload["fill_events"]:
        assert "reason_zh" in payload["fill_events"][0]

    r2 = client.get(f"/runs/{rid}/metrics")
    assert r2.status_code == 200
    m = r2.json()
    assert m["runtime_id"] == rid
    assert "current" in m and "candidate" in m
