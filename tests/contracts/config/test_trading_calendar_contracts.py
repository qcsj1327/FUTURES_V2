...
from config.instrument_universe import default_symbols, trading_sessions_for
...
def test_default_trading_sessions_cover_live_universe_and_close_after_night_session() -> None:
    sessions = trading_sessions_for(default_symbols())
    assert set(sessions) == set(default_symbols())
    ...
    calendar = TradingCalendar(
        sessions_by_symbol={
            ...
            for symbol, symbol_sessions in sessions.items()
        }
    )

    assert calendar.is_trading_time("SHFE.au2606", _ts("2026-05-04T21:30:00"))
    assert calendar.is_trading_time("KQ.m@SHFE.cu", _ts("2026-05-04T23:30:00"))
    assert not calendar.is_trading_time("SHFE.au2606", _ts("2026-05-05T03:00:00"))
    assert not calendar.is_trading_time("KQ.m@SHFE.rb", _ts("2026-05-05T00:30:00"))
